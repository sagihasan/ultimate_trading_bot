# run_all.py – קובץ אחד שמאחד את MAIN ו-SCHEDULER
...
def run_main():
    try:
        ...
        try:
            start_session = (now - timedelta(days=1)).date()
            end_session = (now + timedelta(days=1)).date()
            sessions = nyse.sessions_in_range(start_session, end_session)
        ...
        while True:
            ...
            if now_time == signal_time:
                ...
                message = f"{best_signal}\n"

                if half_day:
                    message += (
                        "\nהיום יום מסחר מקוצר – סגירה ב־13:00 ניו-יורק / 20:00 ישראל.\n"
                        "הבוט קובע: לא להיכנס לעסקה. זיהוי מטרה בוצע – תנאים לא מבצעיים."
                    )
                elif is_dst_gap:
                    message += (
                        "\nתקופת פערי שעון בין ישראל לארה\"ב – סיום מסחר ב־16:00 ניו-יורק / 23:00 ישראל.\n"
                        "הבוט קובע: לא להיכנס לעסקה. זיהוי מטרה בוצע – חלון מבצעי סגור."
                    )
                else:
                    message += "הבוט קובע: להיכנס לעסקה. זיהוי מטרה בוצע. פקודה יוצאת לדרך."

                send_discord_message(DISCORD_PUBLIC_WEBHOOK, message, message_type="signal_main")
                break
